    ...
    <div class="relative">
      @if (expanded()) {
        <button type="button" (click)="toggle()"
          [attr.aria-haspopup]="'listbox'" [attr.aria-expanded]="open()"
      ...
      @if (open()) {
        ...
        <div role="listbox" [attr.aria-label]="'COMMON.LANGUAGE' | translate"
            ...
            <button type="button" role="option"
              ...
              @if (l.code === lang.currentLanguage()) {
                <span class="material-symbols-outlined text-slate-900 dark:text-white"
                  style="font-size: 16px; line-height: 1">check</span>
              }
            </button>
          }
        </div>
