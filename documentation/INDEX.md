...
- [Admin-Bestellungen — Geschäftstag-Filter + Status-Änderung](admin-bestellungen-geschaeftstag-filter.md) — 2026-05-23 — Admin-Bestellliste filtert standardmäßig nach offenen Geschäftstagen (`businessDayId $in` der `status:open`-Tage) statt Kalenderdatum (neuer Default-Filter „Aktueller Geschäftstag", Kalenderfilter bleiben); Detail-Ansicht erlaubt Status-Änderung (alle 4 Status, `PATCH /orders/:id`); RBAC `orders:UPDATE` für TENANT_OWNER + TENANT_MANAGER. POS-Client unverändert. i18n `ORDERS.TIME_BUSINESS_DAY`/`CHANGE_STATUS`
