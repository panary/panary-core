  ...
  template: `
    <div class="flex flex-col w-full">
      ...
      <div class="flex flex-col items-center gap-3 px-6 pt-7 pb-4">
        ...
        <h2 class="text-xl font-bold text-slate-800 text-center">{{ data.title }}</h2>
      </div>

      <!-- Body -->
      <div class="px-6 pb-2 text-center">
        <p class="text-slate-500 text-sm leading-relaxed">{{ data.message }}</p>
        @if (data.detail) {
          <p class="mt-2 text-slate-700 font-semibold text-base">{{ data.detail }}</p>
        }
      </div>
      ...
      <div class="flex flex-col gap-2 px-6 pb-6 pt-4">
        ...
        <button
          (click)="cancel()"
          class="h-12 w-full rounded-xl font-medium text-sm text-slate-500 bg-slate-100 hover:bg-slate-200 transition-all active:scale-95">
          Abbrechen
        </button>
      </div>
    </div>
