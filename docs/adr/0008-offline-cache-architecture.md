...
description: 'Schlanker Client-Cache des cloud-direkten POS im Connect-Tier: IndexedDB via idb hinter austauschbarem Storage-Port, namespaced Tenant-Isolation und Wipe-on-Mismatch-Versionierung.'
tags: [sync, orders, products, devices, offline-cache, pos]
status: stable
decision: accepted
implementation: 'implementiert (Phasen 1–6); zurückgestellt: Profiling/Web-Worker, Asset-Stub, Multi-Replica-Zähler'
...
- **Belegnummer-Reconcile-Kontrakt** (cross-repo, **panary-cloud**): neuer Hook
  ...
  **re-gestempelt**). `fromSync`-Edge-Orders behalten ihre Nummer. Damit landet keine provisorische
  ...
  `panary-cloud/docs/domains/connect-tier-belegnummer-reconcile.md`. Reine api-cloud-Änderung
