      ...
      <div [class]="selectedId() ? 'w-72 shrink-0 border-r border-gray-800' : 'flex-1'"
        ...
        <div class="p-6 space-y-4">
          ...
          @if (loading()) {
            ...
            <p class="text-gray-500 text-center py-12 text-sm">Keine Benutzer</p>
          } @else {
                ...
                <tbody>
                  @for (user of users(); track user._id) {
                    <tr (click)="selectItem(user._id)"
                      ...
                      @if (!selectedId()) {
                        <td class="px-3 py-2.5 text-gray-400 text-xs">{{ user.loginname }}</td>
                        ...
                        <td class="px-3 py-2.5">
                          <span [class]="user.status === 'ACTIVE' ? 'text-green-400 text-xs' : 'text-gray-500 text-xs'">
                            {{ user.status }}
                          </span>
                        </td>
                      }
