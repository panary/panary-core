    ...
    <div class="flex flex-col h-full w-full bg-white dark:bg-gray-900 rounded-xl overflow-hidden">
      ...
      <div class="flex justify-between items-center p-4 border-b border-slate-200 dark:border-gray-700">
        <div class="flex flex-row gap-2 items-center">
          ...
          @if (formattedTime()) {
            <span class="text-xl font-bold px-2 py-0.5 rounded bg-slate-800 text-white">{{ formattedTime() }} Uhr</span>
          }
        </div>
        <button (click)="close()"
          ...
          <mat-step [stepControl]="dateFormGroup">
            ...
            <div class="flex justify-end p-6 pt-0">
              <button matStepperNext [disabled]="dateFormGroup.invalid"
                class="h-12 px-8 text-lg rounded-xl font-bold bg-slate-800 text-white
                       disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-700 transition-colors">
                Weiter
              </button>
            </div>
          </mat-step>
              ...
              @if (!activeField()) {
                <div class="flex justify-end gap-4 mt-auto pt-4">
                  ...
                  <button (click)="submit()" [disabled]="contactFormGroup.invalid"
                    class="h-12 px-8 text-lg rounded-xl font-bold bg-yellow-400 text-slate-900
