    ...
    <div class="flex flex-col h-full bg-white select-none">
      <div class="flex items-center justify-between p-2 mb-2">
        <button (click)="prevMonth()"
          class="w-12 h-12 rounded-xl flex items-center justify-center hover:bg-slate-100 transition-colors">
      ...
      <div class="grid grid-cols-7 mb-2 text-center text-slate-500 font-bold text-sm">
        ...
        @for (date of daysInMonth(); track $index) {
          @if (date) {
            <button
              ...
              [class.bg-slate-100]="!isSelected(date) && !isPast(date)"
              [class.text-slate-700]="!isSelected(date) && !isPast(date)"
