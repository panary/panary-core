...
* [„OHNE"-Modifier — Marker `amount −1`, entfernbare Zutaten und Preisregeln](modifier-ohne-marker.md) - Zwei OHNE-Mechanismen im POS: der Marker amount −1 (preisneutral) und entfernbare Zutaten mit negativem priceAdjustment (Abzug) — beide brauchen Modifier-eigene Schema-Regeln, eine Klemme bei 0 und einen Bon, der sich aufrechnet.
...
* [Tenant-Edge-Replica — projizierte Tenant-Stammdaten am Edge](tenant-edge-replica.md) - Read-only tenants-Service am Edge speichert die Cloud-projizierte Tenant-Sicht (Receipt-Branding, Logo, Localization, Rechtsperson, TSE-Referenzen) via Pull-Sync — der Edge schreibt niemals zurück.
