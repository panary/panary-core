...
* **Creation**: [„OHNE"-Modifier — Marker `amount −1`, Preisregel und Bon-Darstellung](domains/modifier-ohne-marker.md) — die Inline-Feld-Härtung vom 2026-05-22 setzte `minimum: 0` auf `genericLineItemSchema.amount` und lehnte damit jede Bestellung mit OHNE-Modifier als `400` ab (Edge, POS-Offline-Outbox und Cloud-Sync-Push gleichermaßen); neues `modifierLineItemSchema` erlaubt −1 ausschließlich auf Modifier-Ebene, und OHNE ist jetzt in beiden Brutto-Pfaden preisneutral (vorher zog es den Aufpreis ab, während der Bon die Zeile ohne Betrag druckte).
