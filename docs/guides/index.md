...
* [Test-Target an einer Nx-Lib nachrüsten](lib-vitest-test-target.md) - Anleitung, wie eine Lib ohne `test`-Target eines per Plugin-Inferenz bekommt — inklusive der vier Fallen (Config-Drift, TS5069, fehlender JIT-Compiler, `effect()` ohne Scheduler) und des TestBed-freien Musters für Angular-Klassen.
...
* [Rabatte am laufenden Stack verifizieren](rabatte-live-verifikation.md) - Prüfblatt für den Rabatt-Durchstich Cloud → Sync → Edge → POS inklusive MwSt-Gegenprobe, mit den Schritten, die nur an echter Hardware laufen.
